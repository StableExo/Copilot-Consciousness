...
// ethers namespace reserved for future encoding utilities
...
export function encodeFlashSwapCall(functionName: string, functionArgs: any[]): string {
  ...
  try {
    ...
    logger.debug(
      `${logPrefix} Function call encoded successfully: ${calldata.substring(0, 74)}...`
    );
    return calldata;
  } catch (error: any) {
    logger.error(
      `${logPrefix} Failed to encode function call for ${functionName}: ${error.message}`
    );
    throw new Error(
      `Failed to encode FlashSwap function call for ${functionName}: ${error.message}`
    );
  }
}
