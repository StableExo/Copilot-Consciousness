...
export class DifferentialEngine {
  ...
  assessThreat(differential: PowerDifferential): ThreatAssessment {
    ...
    const imminent =
      differential.offensiveDelta > 0.3 &&
      differential.vulnerabilityDelta > 0.3;
  ...
  shouldIntervene(
    ...
    reasoning.push(`Threat assessment: level=${threat.level.toFixed(2)}, one-sided=${threat.oneSided}`);
  ...
  updateWeights(
    ...
    if (!outcomeWasCorrect) {
      ...
      const deltas = [
        ...
        { name: 'offensive', value: Math.abs(differential.offensiveDelta), weight: this.config.offensiveWeight },
        { name: 'defensive', value: Math.abs(differential.defensiveDelta), weight: this.config.defensiveWeight },
        { name: 'vulnerability', value: Math.abs(differential.vulnerabilityDelta), weight: this.config.vulnerabilityWeight },
        { name: 'agility', value: Math.abs(differential.agilityDelta), weight: this.config.agilityWeight },
      ];
      ...
      if (weakest.name === 'size') this.config.sizeWeight = Math.max(0.05, this.config.sizeWeight - adjustment);
      else if (weakest.name === 'offensive') this.config.offensiveWeight = Math.max(0.05, this.config.offensiveWeight - adjustment);
      else if (weakest.name === 'defensive') this.config.defensiveWeight = Math.max(0.05, this.config.defensiveWeight - adjustment);
      else if (weakest.name === 'vulnerability') this.config.vulnerabilityWeight = Math.max(0.05, this.config.vulnerabilityWeight - adjustment);
      else if (weakest.name === 'agility') this.config.agilityWeight = Math.max(0.05, this.config.agilityWeight - adjustment);
