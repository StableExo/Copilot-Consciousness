...
export class SwapSimulator {
  ...
  private _getDodoPoolContract(poolAddress: string): Contract | null {
    ...
    if (!this.dodoPoolContractCache[lowerCaseAddress]) {
      try {
        this.dodoPoolContractCache[lowerCaseAddress] = new Contract(
          poolAddress,
          DODOV1V2_POOL_ABI,
          this.provider
        );
  ...
  public async simulateSwap(
    ...
    try {
      switch (poolState.dexName?.toLowerCase()) {
        ...
        default:
          return {
            success: false,
            amountOut: null,
            error: `Unsupported dex: ${poolState.dexName}`,
          };
      }
    } catch (error: unknown) {
      ...
      return {
        success: false,
        amountOut: null,
        error: `Unexpected error during simulation dispatch: ${message}`,
      };
    }
  }

  private async simulateV3Swap(
    poolState: PoolState,
    tokenIn: Token,
    amountIn: bigint
  ): Promise<SwapSimulationResult> {
    ...
    const tokenOut =
      tokenIn.address.toLowerCase() === token0.address.toLowerCase() ? token1 : token0;
    ...
    try {
      const quoteResult = await (this.quoterContract as any).quoteExactInputSingle.staticCall(
        params
      );
  ...
  private async simulateDodoSwap(
    poolState: PoolState,
    tokenIn: Token,
    amountIn: bigint,
    ...
    if (!baseTokenAddress)
      return { success: false, amountOut: null, error: 'Missing baseTokenAddress' };
    ...
    try {
      ...
      const reason =
        (error as { reason?: string }).reason ||
        (error instanceof Error ? error.message : String(error));
      if (reason.includes('BALANCE_NOT_ENOUGH'))
        return { success: false, amountOut: null, error: reason };
