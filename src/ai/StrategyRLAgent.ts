...
export class StrategyRLAgent extends EventEmitter {
   ...
   */
  private selectBestAction(stateKey: string, currentParams: StrategyParameters): StrategyParameters {
    const stateActions = this.qTable.get(stateKey);
  ...
  private exploreParameters(baseParams: StrategyParameters): StrategyParameters {
    ...
    for (let i = 0; i < paramsToMutate; i++) {
      ...
      if (paramKey === 'priorityFeeStrategy') {
        const strategies: ('conservative' | 'moderate' | 'aggressive')[] = ['conservative', 'moderate', 'aggressive'];
  ...
  private generateRationale(
    ...
    if (Math.abs(suggestedParams.minProfitThreshold - currentParams.minProfitThreshold) > 0.01) {
      const direction = suggestedParams.minProfitThreshold > currentParams.minProfitThreshold ? 'increase' : 'decrease';
      changes.push(`${direction} profit threshold to ${suggestedParams.minProfitThreshold.toFixed(4)} ETH`);
    }
    
    if (Math.abs(suggestedParams.mevRiskSensitivity - currentParams.mevRiskSensitivity) > 0.05) {
      const direction = suggestedParams.mevRiskSensitivity > currentParams.mevRiskSensitivity ? 'increase' : 'decrease';
      changes.push(`${direction} MEV risk sensitivity to ${suggestedParams.mevRiskSensitivity.toFixed(2)}`);
    }
    ...
    return `Suggest: ${changes.join(', ')} based on ${this.episodeCount} episodes (confidence: ${(confidence * 100).toFixed(0)}%)`;
  }
