...
export class SushiV3Builder {
  ...
  static buildParams(
    ...
    const typeString =
      'tuple(address pool1, address pool2, uint24 fee1, uint24 fee2, address tokenIn, address tokenIntermediate, address tokenOut, uint256 borrowAmount, uint256 minAmountOutHop1, uint256 minAmountOutHop2, address titheRecipient, uint8 dexType)';
  ...
  private static validateFee(fee: number): void {
    if (typeof fee !== 'number' || fee < 0 || fee > UINT24_MAX) {
      throw new Error(`SushiV3Builder: Fee must be a valid uint24 (0 to ${UINT24_MAX})`);
    }
  }
