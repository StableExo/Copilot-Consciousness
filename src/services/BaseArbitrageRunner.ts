...
export class BaseArbitrageRunner extends EventEmitter {
  ...
  private async runCycle(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[BaseArbitrageRunner] === Cycle #${this.cycleCount} Complete (${cycleDuration}ms) ===\n`
      );
  ...
  private async scanWithMultiDex(poolData: any[]): Promise<OpportunityResult> {
    ...
    console.log(
      '[BaseArbitrageRunner] Using MultiDexPathBuilder for advanced opportunity detection...'
    );
    ...
    if (this.nnScorer && this.config.enableML) {
      ...
      console.log(
        `[BaseArbitrageRunner] ML Score: ${(mlScore * 100).toFixed(1)}% - ${mlRecommendation}`
      );
    ...
    const traditionalRecommendation =
      netProfit >= this.config.minProfitThresholdEth && mevRisk < this.config.mevRiskThreshold;
  ...
  private async fetchPoolPrices(): Promise<any[]> {
    ...
    console.log(
      `[BaseArbitrageRunner] Fetching prices for ${this.config.targetPools.length} pools...`
    );
  ...
  private shouldExecute(opportunity: OpportunityResult): boolean {
    ...
    // Check MEV risk
    if (
      this.config.enableMevProtection &&
      (opportunity.mevRisk || 0) > this.config.mevRiskThreshold
    ) {
  ...
  private async executeArbitrage(opportunity: OpportunityResult): Promise<void> {
    ...
    try {
      ...
      if (isNonce) {
        console.log(
          `[BaseArbitrageRunner] [${executionId}] Nonce error detected - NonceManager will auto-resync`
        );
  ...
  private async executeWithFlashLoan(
    ...
    if (result.success) {
      ...
      this.recordExecution({
        ...
        opportunity: {
          ...opportunity,
          features: this.config.enableML
            ? this.extractOpportunityFeatures(
                (opportunity as any).arbitragePath || {},
  ...
  private extractOpportunityFeatures(
    ...
    return {
      ...
      pathComplexity:
        (path.swapSteps?.length || 2) * (opportunity.arbType === 'triangular' ? 1.5 : 1.0),
      gasEstimate,
  ...
  async getStrategyVariants(): Promise<any> {
    ...
    console.log(
      `[BaseArbitrageRunner] Evolution engine proposed ${variants.length} strategy variants`
    );
