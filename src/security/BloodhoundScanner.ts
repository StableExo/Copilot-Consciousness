...
export class BloodhoundScanner extends EventEmitter {
  ...
  private initializePatterns(): void {
    this.patterns = [
      ...
      // API Keys (generic patterns)
      {
        type: 'api_key',
        pattern:
          /(?:api[_-]?key|apikey|access[_-]?key|secret[_-]?key)[\s:=]+['""]?([a-zA-Z0-9_-]{20,})['""]?/gi,
        confidence: 0.85,
        severity: 'high',
      },
  ...
  private redactValue(value: string, type: SecretType): string {
    ...
    if (type === 'api_key' || type === 'jwt') {
      return `${value.substring(0, 8)}...[${value.length - 16} chars]...${value.substring(
        value.length - 8
      )}`;
    }
  ...
  private generateSecretRecommendation(type: SecretType): string {
    const recommendations: Record<SecretType, string> = {
      private_key:
        'Move to secure vault (e.g., HashiCorp Vault). Use environment variables. Never commit to version control.',
      mnemonic:
        'Store in hardware wallet or secure vault. Never expose in code or logs. Rotate immediately if exposed.',
      api_key:
        'Use environment variables or secrets manager. Rotate key immediately. Review access logs.',
      ...
      bearer_token:
        'Rotate token immediately. Use short-lived tokens. Implement token refresh mechanism.',
      ...
      rpc_url:
        'Use environment variables. Consider if endpoint contains sensitive project ID. Rotate if needed.',
      ...
      certificate:
        'Revoke certificate if exposed. Issue new certificate. Review certificate usage.',
