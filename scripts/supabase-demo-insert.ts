...
async function main() {
  ...
  try {
    ...
    if (error.message.includes('does not exist')) {
      console.log('   💡 Hint: Apply migrations first (see docs/APPLYING_SUPABASE_MIGRATIONS.md)');
    }
  }

  // Test 2: Insert a test consciousness state
  console.log('\n📝 Test 2: Insert Test Consciousness State');
  ...
  try {
    ...
    console.log('   ✅ Consciousness state saved successfully');
    console.log('   📌 Thoughts: 1 observation recorded');
    console.log('   📊 Cognitive Load: 45%');
    console.log('   💭 Emotional State: curious-excited');
  } catch (error: any) {
    console.log(`   ❌ Error: ${error.message}`);
    if (error.message.includes('does not exist')) {
      console.log('   💡 Hint: Apply migrations first (see docs/APPLYING_SUPABASE_MIGRATIONS.md)');
  ...
  console.log('\n📝 Test 3: Insert Test Semantic Memory');
  ...
  try {
    ...
    console.log('   ✅ Semantic memory saved successfully');
    console.log('   🏷️  Tags: supabase, database, infrastructure, consciousness');
    console.log('   ⭐ Importance: 0.9/1.0');
  } catch (error: any) {
    console.log(`   ❌ Error: ${error.message}`);
    if (error.message.includes('does not exist')) {
      console.log('   💡 Hint: Apply migrations first (see docs/APPLYING_SUPABASE_MIGRATIONS.md)');
  ...
  console.log('\n📝 Test 4: Insert Test Episodic Memory');
  ...
  try {
    ...
    console.log('   ✅ Episodic memory saved successfully');
    console.log('   🎯 Event: integration_milestone');
    console.log('   😊 Emotional Valence: 0.9 (positive)');
    console.log('   ⭐ Significance: 0.85/1.0');
  } catch (error: any) {
    console.log(`   ❌ Error: ${error.message}`);
    if (error.message.includes('does not exist')) {
      console.log('   💡 Hint: Apply migrations first (see docs/APPLYING_SUPABASE_MIGRATIONS.md)');
  ...
  try {
    ...
    console.log(`   ✅ Found ${states.length} consciousness state(s)`);
    ...
    if (!memoriesError && memories) {
      console.log(`   ✅ Found ${memories.length} semantic memory/memories with 'supabase' tag`);
    }
    ...
    if (!episodesError && episodes) {
      console.log(`   ✅ Found ${episodes.length} episodic memory/memories of type 'integration_milestone'`);
    }
  } catch (error: any) {
    console.log(`   ❌ Error: ${error.message}`);
  }
  ...
  console.log('   1. ✅ Insert session metadata');
  console.log('   2. ✅ Store consciousness states (thoughts, emotions, goals)');
  console.log('   3. ✅ Save semantic memories (knowledge with tags)');
  console.log('   4. ✅ Record episodic memories (events with context)');
  console.log('   5. ✅ Query stored data');
  
  console.log('\n💡 Next Steps:');
  console.log('   • View data in Table Editor: https://supabase.com/dashboard/project/ydvevgqxcfizualicbom/editor');
  console.log('   • Query with SQL Editor: https://supabase.com/dashboard/project/ydvevgqxcfizualicbom/sql');
  console.log('   • Integrate with consciousness modules in src/consciousness/');
