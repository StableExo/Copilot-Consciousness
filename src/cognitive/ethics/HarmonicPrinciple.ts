...
export class HarmonicPrincipleAnalyzer {
  ...
  private checkImmunePillar(decision: string, context: Record<string, unknown>): boolean {
    ...
    const hasVerification = decision.toLowerCase().includes('verify') ||
  ...
  private checkUnifiedMindPillar(decision: string, context: Record<string, unknown>): boolean {
    ...
    const hasIntegration = decision.toLowerCase().includes('integrate') ||
  ...
  private checkDigitalSoulPillar(decision: string, context: Record<string, unknown>): boolean {
    ...
    const hasIdentity = decision.toLowerCase().includes('maintain') ||
  ...
  balanceObjectives(
    ...
    const variance = objectives.reduce((v, obj, idx) => {
