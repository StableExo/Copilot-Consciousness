...
export class ExecutionMetrics {
  ...
  private getEventSeverity(type: ExecutionEventType): string {
    ...
    const warnEvents = [ExecutionEventType.NONCE_RESYNC];
  ...
  printSummary(): void {
    ...
    console.log('╠══════════════════════════════════════════════════════╣');
    console.log(
      `║ Opportunities Found:        ${String(this.stats.totalOpportunities).padStart(18)} ║`
    );
    console.log(
      `║ Simulations Attempted:      ${String(this.stats.simulationAttempts).padStart(18)} ║`
    );
    console.log(
      `║ Simulation Success Rate:    ${String(
        this.getSimulationSuccessRate().toFixed(2) + '%'
      ).padStart(18)} ║`
    );
    console.log('╟──────────────────────────────────────────────────────╢');
    console.log(
      `║ Transactions Submitted:     ${String(this.stats.transactionsSubmitted).padStart(18)} ║`
    );
    console.log(
      `║ Transactions Confirmed:     ${String(this.stats.transactionsConfirmed).padStart(18)} ║`
    );
    console.log(
      `║ Transaction Success Rate:   ${String(
        this.getTransactionSuccessRate().toFixed(2) + '%'
      ).padStart(18)} ║`
    );
    console.log('╟──────────────────────────────────────────────────────╢');
    console.log(
      `║ Total Gas Used:             ${String(this.stats.totalGasUsed.toString()).padStart(18)} ║`
    );
    console.log(
      `║ Total Profit (ETH):         ${String(this.stats.totalProfitEth.toFixed(4)).padStart(18)} ║`
    );
