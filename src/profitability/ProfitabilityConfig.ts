...
export function meetsProfitRequirements(
  ...
  if (inputAmount > BigInt(0)) {
    ...
    if (percentageProfit < config.percentageMin) {
      return {
        meets: false,
        reason: `Profit percentage ${percentageProfit.toFixed(3)}% below minimum ${config.percentageMin}%`,
      };
    }
  }
  
  // Check gas multiplier (profit must be at least gasMultiplier * gasCost)
  const minProfitFromGas = estimatedGasCost * BigInt(Math.floor(config.gasMultiplier * 100)) / BigInt(100);
