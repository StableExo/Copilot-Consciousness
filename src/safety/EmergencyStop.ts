...
export class EmergencyStop extends EventEmitter {
   ...
   */
  registerShutdownCallback(name: string, callback: () => Promise<void>, priority: number = 50): void {
    this.shutdownCallbacks.push({ name, callback, priority });
    this.shutdownCallbacks.sort((a, b) => b.priority - a.priority);
    logger.debug(`[EmergencyStop] Registered shutdown callback: ${name} (priority: ${priority})`, 'SAFETY');
  }
  ...
  private async executeShutdown(): Promise<void> {
    ...
    const shutdownPromises = this.shutdownCallbacks.map(async ({ name, callback }) => {
      try {
        ...
        logger.error(
          `[EmergencyStop] Shutdown callback failed: ${name} - ${error instanceof Error ? error.message : String(error)}`,
          'SAFETY'
        );
      }
    });
  ...
  async recover(approvedBy?: string): Promise<boolean> {
    ...
    logger.info(`[EmergencyStop] Attempting recovery (approved by: ${approvedBy || 'system'})`, 'SAFETY');
  ...
  updateHealthScore(score: number): void {
    if (this.config.enableAutoStop && 
        score < this.config.minHealthScore && 
