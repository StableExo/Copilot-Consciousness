...
export class LayerStackManager {
  ...
  updateLayerValidation(category: number, layerNumber: number, success: boolean): Layer {
    ...
    const layers = stack.layers.map((l) => (l.layerNumber === layerNumber ? updatedLayer : l));
   ...
   */
  lockLayerAfterValidation(
    category: number,
    layerNumber: number,
    minValidations: number = 10
  ): Layer {
    const layer = this.getLayer(category, layerNumber);
    if (!layer) {
      throw new Error(`Layer ${layerNumber} in category ${category} not found`);
    ...
    const layers = stack.layers.map((l) => (l.layerNumber === layerNumber ? lockedLayer : l));
  ...
  queryLayers(query: LayerQuery): readonly Layer[] {
    ...
    if (query.tags && query.tags.length > 0) {
      allLayers = allLayers.filter(
        (l) => l.tags && query.tags!.some((tag) => l.tags!.includes(tag))
      );
    }
