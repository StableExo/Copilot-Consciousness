...
export class SecurityPatternLearner extends EventEmitter {
  ...
  constructor(config?: Partial<PatternLearnerConfig>) {
    ...
    console.log('[SecurityPatternLearner] Initialized with automatic learning:', this.config.enableAutomaticLearning);
  }
  ...
  async recordIncident(incident: SecurityIncident): Promise<void> {
    console.log(`[SecurityPatternLearner] Recording incident: ${incident.incidentId} (${incident.type})`);
  ...
  private async detectCompoundPatterns(incident: SecurityIncident): Promise<void> {
    ...
    if (existingPattern) {
      ...
      const pattern: SecurityPattern = {
        ...
        detectedAt: incident.timestamp,
        type: 'attack_pattern',
        description: `Compound attack involving: ${sortedTypes.map(t => t.replace(/_/g, ' ')).join(', ')}`,
        occurrences: 1,
        firstSeen: incident.timestamp,
        lastSeen: incident.timestamp,
        characteristics: {
  ...
  private async clusterIncidents(): Promise<void> {
    ...
    for (const [threatType, incidents] of incidentsByType.entries()) {
      ...
      const avgSeverity = incidents.reduce((sum, inc) => 
        sum + (severityMap[inc.severity] ?? 2), 0) / incidents.length;
      
      const totalImpact = incidents.reduce((sum, inc) => 
        sum + (inc.impacted?.loss ?? 0), 0);
  ...
  private createMitigationSuggestion(cluster: IncidentCluster): MitigationSuggestion {
    ...
    const confidence = Math.min(1.0, 0.5 + (cluster.incidents.length / 20) + (cluster.pattern.riskScore * 0.3));
    
    // Estimate impact
    const expectedImpact = Math.min(1.0, cluster.pattern.riskScore * (cluster.incidents.length / 10));
    ...
    return {
      ...
      description: `Based on ${cluster.incidents.length} incidents with ${cluster.avgSeverity.toFixed(1)} avg severity and $${cluster.totalImpact.toFixed(0)} total impact`,
      priority,
  ...
  private createPatternMitigationSuggestion(pattern: SecurityPattern): MitigationSuggestion {
    ...
    return {
      ...
      description: `Pattern detected ${pattern.occurrences} times with ${(pattern.riskScore * 100).toFixed(0)}% risk score`,
      priority,
      implementationSteps: steps,
  ...
  private generateMitigationSteps(threatType: ThreatType, pattern: SecurityPattern): string[] {
    ...
    if (pattern.characteristics.temporal) {
      steps.push(`Increase security during high-risk time periods (hour ${pattern.characteristics.hour})`);
    }
